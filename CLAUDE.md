...
- ✅ **Phase 128 (v0.6.83) — 연속입력 큐 메시지 간헐 증발 수정** (2026-06-04): K 의 "연속입력 시 입력한 내용이 한번씩 적용 안 됨" 보고. **근본 원인 = flush effect 의 self-cancel 레이스**: 작업(streaming) 중 Enter 친 메시지는 `queuedSend` 에 적재됐다가 streaming 종료 시 `App.tsx` 의 flush `useEffect`(deps `[isStreaming, queuedSend, handleSendMessage]`)가 자동 전송. 그런데 그 effect 가 ① `t1=setTimeout(send, 0)` 예약 + cleanup `()=>clearTimeout(t1)` 반환 ② 같은 body 에서 `setQueuedSend(null)` 호출 → 즉시 re-render → 의존성 `queuedSend` 변경 → React 가 **이전 effect 의 cleanup(`clearTimeout(t1)`)을 새 effect 전에 실행** → in-flight flush 타이머 t1 이 죽어 큐 메시지 증발. `setTimeout(0)`(매크로태스크) vs React passive-effect flush(MessageChannel, 더 높은 우선순위) 경쟁이라 **타이밍 따라 "한번씩"** 누락. **수정**: 타이머를 `flushTimerRef` ref 로 보관하고 **flush effect 의 cleanup 에서 clearTimeout 제거** → 의존성 변경 cleanup 이 진행 중 flush 를 취소 못 함. 타이머 정리는 **언마운트 전용 effect** 에서만. + `flushTimerRef.current !== null` 재진입 가드(이중 방어). 일반화: **effect 내부에서 자기 의존성 state 를 set + 그 effect 의 cleanup 이 in-flight 작업을 취소**하면 self-cancel 레이스. 타이머/구독의 lifecycle 은 "의존성 cleanup" 이 아니라 "언마운트 cleanup" 또는 ref 로 분리해야.
- ✅ **Phase 129 (v0.6.84) — VM/원격 호환 모드 (WebView2 GPU 가속 OFF)** (2026-06-04): K 가 "타 PC 의 VM 에서 작업 시 로컬 이미지/영상 미리보기가 안 보임" 보고. **근본 원인 = VM 의 가상 GPU 에서 WebView2(Chromium) GPU 합성 실패** → data URL 이미지가 디코드·로드는 됐지만 화면에 페인트가 안 돼 "빈칸"으로 표시(스크롤/리사이즈 같은 repaint 트리거 전까지). 미리보기 경로 자체는 정상(`Message.tsx` 가 `loadPreview`→fs.readFile→base64 data URL, GPU 무관 로직)이라 코드 버그가 아니라 **렌더 환경 문제**. **해결**: WebView2 에 `--disable-gpu --disable-gpu-compositing` 인자 주입해 software 렌더 강제. env `WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS` 는 webview 생성 전에 읽혀야 하므로 `lib.rs::run_with_options()` **최상단**(Builder 이전)에서 flag 파일(`data_root()/webview-gpu-off.flag`) 존재 시 `std::env::set_var`. 토글은 `get/set_webview_gpu_off` 커맨드 + Settings system 탭 "VM/원격 호환 모드" 토글(per-PC flag 파일 → **2PC 중 VM PC 만 켜고 메인 PC 는 GPU 가속 유지** 가능, 재시작 후 적용). 일반화: VM/RDP 에서 "콘텐츠가 로드는 되는데 안 그려짐(빈칸/검정)" = 거의 항상 GPU 합성 문제 → `--disable-gpu` 가 정답. 코드 미리보기 경로 의심 전에 렌더 환경(가상 GPU) 먼저 확인.
