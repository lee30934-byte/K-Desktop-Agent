...
interface AgentFlags {
  ...
  // ON 이면 폴더(프로젝트)에 부여된 projectProfile(금지 도구·메모리 태그·기본 경로)이
  // 도구 게이트/메모리 로딩/시스템 텍스트에 enforce 됨. OFF 면 projectProfile 무시 → 종전 동작.
  projectMode: boolean;
}
...
const FLAG_GATED_TOOLS: Record<keyof AgentFlags, string[]> = {
  ...
  // projectMode 는 별도 도구를 노출/숨김하지 않음 — 프로젝트별 금지 도구는
  // projectProfile.forbiddenTools 로 동적 결정되므로 정적 게이트 목록은 비움.
  projectMode: [],
};
...
/**
 * Phase 138 (v0.7.10) — #3 프로젝트 모드: 프로젝트별 금지 도구 해석.
 * projectMode flag OFF 또는 profile/forbiddenTools 없으면 빈 배열 → 종전 동작.
 * ON + forbiddenTools 존재 시 정규화된 풀네임 배열 반환 (buildToolFlags 로 병합).
 */
function resolveProjectForbidden(
  profile: ProjectProfile | undefined,
  flags: AgentFlags,
): string[] {
  if (!flags.projectMode || !profile || !Array.isArray(profile.forbiddenTools)) return [];
  return profile.forbiddenTools
    .filter((t): t is string => typeof t === "string" && t.trim().length > 0)
    .map((t) => t.trim());
}

/** projectMode ON + memoryTags 지정 시 정규화된 태그 배열 반환 (없으면 빈 배열 → 필터 미적용). */
function resolveProjectMemoryTags(
  profile: ProjectProfile | undefined,
  flags: AgentFlags,
): string[] {
  if (!flags.projectMode || !profile || !Array.isArray(profile.memoryTags)) return [];
  return profile.memoryTags
    .filter((t): t is string => typeof t === "string" && t.trim().length > 0)
    .map((t) => t.trim().toLowerCase());
}

/**
 * Phase 138 (v0.7.10) — #3 프로젝트 모드: 시스템 텍스트에 박을 프로젝트 컨텍스트 블록.
 * projectMode flag OFF 또는 profile 없으면 빈 문자열 → 종전 동작.
 * name/defaultPath/memoryTags/forbiddenTools 중 채워진 것만 안내.
 */
function buildProjectProfileBlock(
  profile: ProjectProfile | undefined,
  flags: AgentFlags,
): string {
  if (!flags.projectMode || !profile) return "";
  const lines: string[] = [];
  const name = typeof profile.name === "string" ? profile.name.trim() : "";
  const defaultPath = typeof profile.defaultPath === "string" ? profile.defaultPath.trim() : "";
  const forbidden = resolveProjectForbidden(profile, flags);
  const tags = resolveProjectMemoryTags(profile, flags);
  if (!name && !defaultPath && forbidden.length === 0 && tags.length === 0) return "";
  lines.push("", "", "[프로젝트 모드]");
  lines.push(
    "이 대화는 특정 프로젝트에 격리되어 있습니다. 아래 범위(scope)를 벗어나는 작업·경로·도구는",
    "K 의 명시적 재확인 없이 수행하지 마세요. 다른 프로젝트의 맥락을 이 대화로 끌어오지 마세요.",
  );
  if (name) lines.push(`- 프로젝트: ${name}`);
  if (defaultPath) {
    lines.push(
      `- 기본 작업 경로: ${defaultPath}`,
      "  (경로가 모호하면 이 경로를 기준으로 삼고, 이 밖의 경로를 건드릴 땐 먼저 K 에게 알리세요.)",
    );
  }
  if (forbidden.length > 0) {
    lines.push(
      `- 이 프로젝트 금지 도구 (사용 금지): ${forbidden.join(", ")}`,
    );
  }
  if (tags.length > 0) {
    lines.push(
      `- 메모리 범위: ${tags.join(", ")} 태그 + 공용 메모리만 로딩됩니다 (타 프로젝트 메모리 비노출).`,
    );
  }
  return lines.join("\n");
}
...
function buildEngineSystemText(
  ...
  opts?: { compact?: boolean; projectProfile?: ProjectProfile },
): string {
  ...
  // Phase 138 (v0.7.10) — #3 프로젝트 모드 블록 (projectMode OFF 면 빈 문자열 → 종전 동작).
  const projectProfileBlock = buildProjectProfileBlock(opts?.projectProfile, agentFlags);
...
interface MemoryFileMeta {
  ...
  // Phase 138 (v0.7.10) — #3 프로젝트 모드: frontmatter `projects:` 태그 목록.
  // 비어 있으면 "공용 메모리" (모든 프로젝트에 로딩). 지정 시 활성 projectMemoryTags 와
  // 교집합 있을 때만 본문 로딩 → 타 프로젝트 메모리 누출 차단.
  projects: string[];
}
...
function parseMemoryFrontmatter(body: string): MemoryFileMeta {
  ...
  return {
    ...
    hasFrontmatter: true,
    projects: extractYamlList(block, "projects").map((p) => p.toLowerCase()),
  };
}
...
function loadMemoryContext(
  userMessage?: string,
  // Phase 138 (v0.7.10) — #3 프로젝트 모드: 활성 프로젝트의 메모리 태그.
  ...
  const activeTags = Array.isArray(projectMemoryTags)
    ? projectMemoryTags.filter((t) => typeof t === "string" && t.trim()).map((t) => t.trim().toLowerCase())
    : [];
  try {
    const entries: MemorySectionEntry[] = [];
    ...
    if (existsSync(dir)) {
      ...
      for (const f of files) {
        try {
          ...
          // Phase 138 (v0.7.10) — #3 프로젝트 모드 메모리 스코프 필터.
          // activeTags 지정 + 파일에 projects 태그 있음 + 교집합 없음 → 본문 생략, stub 만.
          // projects 태그 없는 파일(공용) 및 isCore 는 필터 면제 (항상 로딩).
          if (
            activeTags.length > 0 &&
            !isCore &&
            meta.projects.length > 0 &&
            !meta.projects.some((p) => activeTags.includes(p))
          ) {
            entries.push({
              group: "memory",
              priority: MEMORY_PRIORITY_SUMMARY,
              file: f,
              text: `### ${f} (타 프로젝트 메모리 — 현재 프로젝트 범위 밖, 미로딩)\n- 소속: ${meta.projects.join(", ")}\n  (이 프로젝트와 무관하면 무시. 필요하면 K 에게 알리고 ${f} 직접 read)`,
            });
            continue;
          }
...
[장기 분리작업 자동 이어가기 — task-watch]
빌드/OCR/렌더링처럼 오래 걸리는 작업을 백그라운드로 돌려놓고 "끝나면 알아서 이어가야" 할 때 사용합니다.
KDA 는 매 턴을 단발성으로 처리하므로, 턴 안에서 sleep 으로 완료를 기다리지 말고 아래 마커를 남긴 뒤 턴을 끝내세요.
KDA 상주 프로세스가 조건 충족을 감지하면 같은 대화창에 새 턴을 자동 주입해 당신을 깨웁니다.
...
등록 방법: \`~/.kda/task-watch/<고유id>.json\` 파일을 하나 생성 (Write 또는 셸 사용). 스키마:
  {
    ...
    "createdAt": "<ISO8601 현재시각>",
    "timeoutMs": 3600000
  }
- watch 는 둘 중 하나: {"type":"file","path":"..."}(그 파일이 생기면 발화) 또는 {"type":"pid","pid":1234}(그 PID 가 종료되면 발화).
- 되도록 file(.done 마커) 방식을 쓰세요. PID 는 재사용 위험이 있습니다. timeoutMs 는 항상 안전망으로 넣으세요.
- 장기 작업은 반드시 이 턴이 끝나도 살아남게 detached 로 실행하고(예: 완료 시 \`.done\` 파일을 쓰도록), 그 파일을 감시 대상으로 지정하세요.
- 등록했으면 "완료되면 이어서 처리하겠다"고 K 님께 한 줄 알리고 턴을 마치면 됩니다.
...
type UserMessage = {
  ...
  // Phase 138 (v0.7.10) — #3 대화별 프로젝트 모드.
  // App.tsx 가 활성 conv 의 folder.projectProfile 을 매 turn 박음.
  // agent-flags.json 의 projectMode 가 ON 일 때만 enforce (OFF 면 무시 → 종전 동작).
  projectProfile?: ProjectProfile;
};

// Phase 138 (v0.7.10) — #3 프로젝트 프로필.
// 폴더(=프로젝트)에 부여하는 스코프 격리 규칙. 모든 필드 optional → 부분 지정 가능.
//   - name:           프로젝트 표시 이름 (시스템 텍스트에 명시 → 모델이 컨텍스트 인지)
//   - forbiddenTools: 이 프로젝트에서 금지할 도구 풀네임 (lockedTools 와 동일하게 disallowed 병합)
//   - memoryTags:     로딩 허용 메모리 태그. 지정 시 memory/*.md frontmatter 의 `projects:` 와
//                     교집합이 있는 파일 + 태그 없는(공용) 파일만 본문 로딩 → 타 프로젝트 메모리 누출 차단.
//   - defaultPath:    이 프로젝트의 기본 작업 경로 (시스템 텍스트에 안내 → 경로 혼동 방지)
...
function buildToolFlags(
  ...
  // Phase 138 (v0.7.10) — #3 프로젝트 모드. projectMode ON 일 때만 호출부가 채워 전달.
  // lockedTools 와 동일하게 disallowed 로 병합 (카테고리 토글과 독립적인 하드 차단).
  projectForbiddenTools?: string[],
): ToolFlags {
  ...
  // Phase 138 (v0.7.10) — #3 프로젝트 모드: 프로젝트별 금지 도구 병합.
  // 호출부가 projectMode ON + projectProfile.forbiddenTools 존재할 때만 채워 전달.
  // lockedTools 와 동일하게 lockedCount 에 합산 (UI/로그 일관성).
  if (projectForbiddenTools && Array.isArray(projectForbiddenTools)) {
    for (const t of projectForbiddenTools) {
      if (typeof t === "string" && t.trim()) {
        disallowed.push(t.trim());
        lockedCount++;
      }
...
async function handleViaClaudeCLI(msg: UserMessage): Promise<void> {
  ...
  // Phase 138 — projectMode ON 이면 프로젝트 메모리 태그로 스코프 필터.
  // (agentFlags 선언은 아래 toolFlags 블록 — 여기선 mtime 캐시된 loadAgentFlags() 인라인 사용.)
  ...
  // Phase 138 (v0.7.10) — #3 프로젝트 모드 게이트는 buildToolFlags 보다 먼저 필요.
  // agent-flags.json 은 mtime 캐시라 여기서 먼저 읽어도 비용 거의 0 (아래에서 재사용).
  const agentFlags = loadAgentFlags();
  const projectForbidden = resolveProjectForbidden(msg.projectProfile, agentFlags);
  const toolFlags = buildToolFlags(
    msg.permissions,
    msg.lockedTools,
    msg.safeMode ?? "off",
    projectForbidden,
  ...
  if (projectForbidden.length > 0) {
    log(
      "info",
      `[ProjectMode] '${msg.projectProfile?.name ?? "(unnamed)"}' 금지 도구 ${projectForbidden.length}개 적용`,
    );
  }
  ...
  // (agentFlags 는 위에서 이미 로드 — projectMode 게이트가 buildToolFlags 보다 먼저 필요했음.)
  ...
  const featureGuidance = buildAgentFeatureGuidance(agentFlags);
  // Phase 138 (v0.7.10) — #3 프로젝트 모드 블록 (projectMode OFF 면 빈 문자열 → 종전 동작).
  const projectProfileBlock = buildProjectProfileBlock(msg.projectProfile, agentFlags);
  const fullSystemPrompt =
    SYSTEM_PROMPT + soulBlock + folderInstructionBlock + projectProfileBlock + askGuidance + manualGuidance + featureGuidance;
...
async function handleViaCodexCLI(msg: UserMessage): Promise<void> {
  ...
  const baseContent = attachmentsGuidance
    ...
    : msg.content;
  // Phase 106 — 현재 메시지를 넘겨 조건부 메모리(triggers) 선택 로딩.
  // Phase 138 — projectMode 스코프 필터 (flag OFF 면 빈 배열 → 종전 동작).
  const memory = loadMemoryContext(
    msg.content,
    resolveProjectMemoryTags(msg.projectProfile, loadAgentFlags()),
  );
  ...
  const codexSystemText = buildEngineSystemText(msg.folderSystemPrompt, codexAgentFlags, {
    ...
    projectProfile: msg.projectProfile,
  });
...
async function handleViaRestAPI(msg: UserMessage, provider: Provider): Promise<void> {
  ...
  // Phase 138 (v0.7.10) — #3 프로젝트 모드 블록 (projectMode OFF 면 빈 문자열).
  const restProjectBlock = buildProjectProfileBlock(msg.projectProfile, restAgentFlags);
  const restSystemPrompt =
    SYSTEM_PROMPT_REST + restSoulBlock + restFeatureGuidance + restProjectBlock + memory.content;
  ...
  const restProjectForbidden = resolveProjectForbidden(msg.projectProfile, restAgentFlags);
  const permFlags = buildToolFlags(
    msg.permissions,
    msg.lockedTools,
    msg.safeMode ?? "off",
