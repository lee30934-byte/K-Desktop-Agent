...
# ─── Phase 24 (v0.5.10): UTF-8 출력 강제 ────────────────
# PS 5.1 기본은 한국어 Windows 에서 stdout/stderr 가 CP949 (EUC-KR) 로 흐름.
# Rust 의 String::from_utf8_lossy(&output.stderr) 가 한글 바이트를 ? 로 fallback → 진짜 에러 안 보임.
# Console.OutputEncoding + $OutputEncoding + chcp 65001 셋 다 강제로 박아 UTF-8 정착.
try {
  [Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false)
  $OutputEncoding = [System.Text.UTF8Encoding]::new($false)
  $null = & cmd /c 'chcp 65001 > nul 2>&1'
} catch {
  # 인코딩 설정 실패해도 본 로직 진행 — 메시지 깨질 수 있지만 동작은 함
}
...
# ─── 0. winget 가용성 검사 (Phase 24 강화) ──────────────
# K-Desktop-Agent 의 sidecar 가 spawn 한 powershell 세션은 PATH 가 부모 (Tauri 본체) 기준이라
# 사용자 세션 PATH 에 있는 winget 이 안 보일 수 있음. WindowsApps 의 정식 설치 경로도 같이 검사.
function Resolve-WingetExe {
  # 1. PATH 에서 직접
  $cmd = Get-Command winget -ErrorAction SilentlyContinue
  if ($cmd) { return $cmd.Source }
  # 2. App Installer 의 정식 위치 (Microsoft.DesktopAppInstaller_*)
  $appData = $env:LOCALAPPDATA
  if ($appData) {
    $direct = Join-Path $appData 'Microsoft\WindowsApps\winget.exe'
    if (Test-Path $direct) { return $direct }
  }
  # 3. WinGet 패키지 설치 위치 (UWP)
  $packageRoot = Join-Path $env:LOCALAPPDATA 'Microsoft\WinGet\Packages'
  if (Test-Path $packageRoot) {
    $found = Get-ChildItem $packageRoot -Filter 'winget.exe' -Recurse -ErrorAction SilentlyContinue | Select-Object -First 1
...
function Install-WingetPackage($id, $displayName) {
  ...
  try {
    $wingetPath = if ($script:WingetExe) { $script:WingetExe } else { 'winget' }
    & $wingetPath install --id $id --silent --accept-package-agreements --accept-source-agreements 2>&1 | ForEach-Object {
...
# Phase 24 (v0.5.10): winget 은 "도구가 없을 때만" 필요한 의존성 — 이미 모든 도구가 깔린
# K 본 PC 에서는 winget 없어도 정상 ready 처리해야 함. universal 동작의 핵심.
$needsWinget = (-not $result.before.node) -or (-not $result.before.git) -or
               ((-not $SkipPython) -and (-not $result.before.python))

if ($needsWinget -and -not $result.before.winget) {
  Out-Err "winget 이 없어요 — 누락된 도구 (Node/Git/Python) 자동 설치 불가."
  Out-Err "Windows 10 1809+ 또는 11 필요. Microsoft Store 에서 'App Installer' 설치 후 재시도."
  Out-Err "또는 K 가 직접 다운로드: nodejs.org / git-scm.com / python.org"
  $result.fatal = 'no winget but tools missing'
  $result.missing = @()
  if (-not $result.before.node)   { $result.missing += 'node' }
  if (-not $result.before.git)    { $result.missing += 'git' }
  if ((-not $SkipPython) -and (-not $result.before.python)) { $result.missing += 'python' }
  if ($AsJson) {
...
if (-not $result.before.winget) {
  Out-Status "  ⚠ winget 없지만 모든 winget 의존 도구가 이미 설치돼 있어 진행 가능" 'Yellow'
}
